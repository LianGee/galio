...
class BuildService:
    def __init__(self, workspace, project: Project, branch, description=None, user=None):
        ...
        self.image_name = f'{ConfigUtil.get_str_property(key=config.HARBOR_APP_REPOSITORY)}/' \
    ...
    def build(self):
        ...
        try:
            ...
            repository = f'{ConfigUtil.get_str_property(key=config.HARBOR_APP_REPOSITORY)}/{self.project.name}'
            DockerService.build(
                self.target_path,
                tag=self.image_name,
                console=self.console,
